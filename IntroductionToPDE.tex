...
\begin{lemma}\label{lem67cc828c}
	...
	\begin{aligned}
		u(x,t) 
		&= \iint_C \bigg( \Phi(x-y,t-s) (\de_s\zeta(y,s)+\laplacian_y\zeta(y,s) )
			+ 2\Diff_y \Phi(x-y,t-s) \Diff_y\zeta(y,s) \bigg) \cdot u(y,s) \did y\did s \\
		&\qquad+ \iint_C \Phi(x-y,t-s) \zeta(y,s) \bigg(\de_su(y,s) - \laplacian_y u(y,s)\bigg) \did y\did s .
	\end{aligned}
	\end{equation}
\end{lemma}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%	\end{align*}
	...
	Moreover,
	\begin{align*}
	\iint_C \Phi(x-y,t-s) \Diff\zeta\cdot\Diff u \did y\did s
...
\begin{proof}
	...
	Hence, 
	\begin{equation}
		\dot e_w(t)^2 \le \ddot e_w(t) \cdot e(t) .
	\end{equation}
	...
	\begin{equation}
		e(1/2 t_1 + 1/2 t_2 ) 
		\le e(t_1)^{1/2} e(t_2)^{1/2} .
	\end{equation}
	Taking the limit to $t_1\to a$ and $t_2\to b$ we obtain $e(\dfrac{a+b}{2}) = 0$, in contradiction with $e_w>0$ on $(a,b)$.
...
\end{equation}
Therefore, if $v=\de_tu - \de_x u$, then $v$ solves the transport equation~\eqref{} (with $b=1$:
\begin{equation}
\begin{cases}
...
By the nonhomogeneous transport equation~\eqref{} (with $b=-1$ and $f=v$), we have
\begin{align*}
 	u(x,t)
...
\begin{lemma}[Euler--Poisson--Darboux equation]\label{lem67cd498e}
	...
	\begin{cases}
		\de_t^2 U - \de_r^2 U - \frac{n-1}{r} \de_t U = 0 & \text{ in }(0,+\infty)\times(0,+\infty) , \\
...
Let $u\in C^2(\R^3\times[0,+\infty))$ be such that~\eqref{eq67cd4bec} holds, that is
\begin{equation}
\begin{cases}
...
\end{cases}
\end{equation}
So, for each $x\in\R^3$, the function $U(x;\cdot,\cdot)$ solves the pinched string problem~\ref{subs67cd4e88}, from which we obtain
\begin{equation}
...
\begin{align*}
	...
	&= \de_r\tilde G(x;t) + \tilde H(x;t) \\
	&= (G(x;t) + t\de_t G(x;t)) + t H(x;t) \\
	&= \fint_{\de B(x,t)} g(y) \did S(y) 
		+ \frac{t^2}{3} \fint_{B(x,t)} \laplacian g(y) \did y 
		+ t \fint_{\de B(x,t)} h(y) \did S(y) \\
	&= \fint_{\de B(x,t)} \left( g(y) + t \Diff g(y)\cdot \frac{y-x}{|y-x|} + th(y) \right) \did S(y) \\
	&= \fint_{\de B(x,t)} \left( g(y) + \Diff g(y)\cdot (y-x) + th(y) \right) \did S(y) .
\end{align*}

\begin{theorem}\label{thm67ce9378}
	...
	\begin{equation}\label{eq67ce93f5}
		u(x,s+t) = \fint_{\de B(x,t)} \left( u(y,s) + \Diff_yu(y,s) \cdot (y-x) + t \de_s u(y,s) \right) \did S(y) .
	\end{equation}
\end{theorem}
\begin{proof}
	•
...
\begin{theorem}
	Let $g\in C^2(\R^3)$ and $h\in C^1(\R^2)$.
	Then there exists a unique $u\in C^2(\R^3\times[0,+\infty))$ such that 
	\begin{equation}
	\begin{cases}
...
\end{theorem}
\begin{proof}
	We take $u$ defined in~\eqref{eq67ce93f5} and then we apply the Proposition about spherical means.
\end{proof}
...
\begin{remark}
	The issue here is that, for $n=2$, the Euler--Poisson--Darboux equation~\eqref{} cannot be transformed into a wave equation.
	(Why?)
\end{remark}
...
\end{cases}
\end{equation}
We define $\tilde u\in C^2(\R^2\times[0,+\infty))$ by $\tilde u(x_1,x_2,x_3,t) = u(x_1,x_2,t)$,
$\tilde g(x_1,x_2,x_3) = g(x_1,x_2)$
and $\tilde h(x_1,x_2,x_3) = u(x_1,x_2)$.
...
denoting by $\tilde x= (x_1,x_2,0)\in \R^3$ for each given $x=(x_1,x_2)\in\R^2$, then
\begin{align*}
	u(x_1,x_2,t)
 	&= \tilde u(x_1,x_2,0,t) 
	= \fint_{\de B_3(\tilde x,t)} \left( g(\tilde y) + \Diff g(\tilde y)\cdot (\tilde y-\tilde x) + th(\tilde y) \right) \did S(\tilde y)  \\
	&\overset{??}= \frac{1}{3\omega_3 t^2} \int_{-t}^t \int_{\de B_2(x,\sqrt{t^2-s^2})}
	...
	&= [...] \\
	&= \frac12 \fint_{B_2(x,r)} \frac{g + th + \Diff g\cdot (y-x) }{ ( t^2 - |y-x|^2 )^{1/2} } \did S(y) .
\end{align*}
