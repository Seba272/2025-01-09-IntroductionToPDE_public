...
\begin{exercise}
	Prove Corollary~\ref{cor67e265dd} using Proposition~\eqref{prop67e267de}.
\end{exercise}
...
\begin{exercise}[Young's inequality]
	Show that, if $f\in L^1(\R^n)$ and $g\in L^{p}(\R^n)$,
	then $f*g\in L^p(\R^n)$ and $\|f*g\|_{L^p} \le \|f\|_{L^1} \|g\|_{L^p}$.
...
\begin{proof}
	...
	\begin{align*}
		...
		[\text{by Lemma~\ref{lem67ea55c8} and Proposition~\ref{prop67ea5631}.1}]
...
\begin{lemma}\label{lem67ea55c8}
	Let $\phi,\psi\in \scr D$.
	For $h>0$, define
	\begin{equation}
		\rho_h(x) = \sum_{z\in\Z^n} h^n \phi(x-hz) \psi(hz) .
	\end{equation}
	Then $\rho_h\in\scr D$ for all $h>0$ and
	$\lim_{h\to0} \rho_h = \phi*\psi$ in $\scr D$.
\end{lemma}
\begin{proof}
	Clearly $\rho_h$ is smooth.
	The support of $\rho_h$ is contained in $\spt(\phi) + \spt(\psi)$ (cfr.~Exercise~\ref{ex67ea4f16}).
	Since the function $(x,y)\mapsto \phi(x-y)\psi(y)$ is uniformly continuous,
	then $\rho_h\to \phi*\psi$ uniformly (see Exercise~\ref{ex67ea5356}).
	...
	By~\eqref{eq67e16d22}, we obtain $\rho_h\to \phi*\psi$ in $\scr D$.
\end{proof}
...
Let $\rho\in C^\infty_c(\R^n)$ be such that $\spt(\rho)=B(0,1)$, $0\le \rho\le 1$, and $\int\rho\did x = 1$.
...
\begin{proof}
	...
	&\overset{\eqref{eq67e28b61}}= \lim_{\epsilon\to0} ((u*\rho_\epsilon)*\phi)(0) \\
	&\overset{\eqref{eq67e288dc}}= \lim_{\epsilon\to0} (u*(\rho_\epsilon*\phi))(0) \\
...
\begin{proof}
	\ci
	We have shown this inclusion in~\ref{subs67eb9b4b}.
	...
	every $A\in\scr D'(\Omega)$ with compact support extends uniquely to a continuous linear functional $\bar A:\scr E(\Omega)\to\C$.
	
	So, let $A\in\scr D'(\Omega)$ be with compact support.
...
We have extended this operation to convolution $u*v$ where $u\in\scr D'$ is a distribution and $v\in\scr D$ is a function.
As it happens with the product, convolution is not defined for arbitrary distributions.
The most general definition is of a convolution $u*v$ with $u,v\in\scr D'$ and {\it at least one of the distributions has compact support}.
...
\begin{proposition}
	If $u\in\scr E'$ and $f\in\scr E$, then $u*f\in\scr E$ and 
	$\spt(u*f) \subset \spt(u) + \spt(f)$.
...
Moreover, by~\eqref{eq67ebace9}, $L$ commute with translation, i.e., $L\circ\tau_v = \tau_v\circ L$ for all $v\in\R^n$.
...
%	We can assume $\phi_\infty=0$: 
	We need to show that $u*\phi_j\to u*\phi_\infty$ in $\scr E$,
	that is, $\Diff^\alpha(u*\phi_j)\to \Diff^\alpha(u*\phi_\infty)$ uniformly on compact sets, for every $\alpha\in\N^n$.
	Since $\Diff^\alpha(u*\phi_j) = u*(\Diff^\alpha\phi_j)$ and since $\phi_j$ is an arbitrary sequence, we only need to show that convergence for $\alpha=0$.
...
\begin{theorem}[{\cite[Thm.4.2.1]{MR717035}}]\label{thm67ebada4}
	...
	\item
	there is $u\in \scr D'$ such that $L\phi = u*\phi$.
	\end{enumerate}
...
\begin{equation}\label{eq67ebaf4c}
	...
	u_1 * (u_2*\phi) 
\end{equation}
is a well defined element of $\scr E$.
Indeed, we have two cases:
\begin{enumerate}
\item if $u_1\in\scr E'$, then we know that $u_2*\phi\in \scr E$ and thus~\eqref{eq67ebaf4c} is well defined;
\item if $u_2\in\scr E'$, then $u_2*\phi\in\scr D$ and thus~\eqref{eq67ebaf4c} is well defined.
\end{enumerate}
Moreover, thanks to Lemma~\ref{lem67ebad94}, $\phi\mapsto L\phi$ is continuous.
By Theorem~\ref{thm67ebada4}, there is a unique $u\in\scr D'$ such that $L\phi = u*\phi$ for all $\phi\in\scr D$.
We set
\begin{equation}
	u = u_1 * u_2
	\quad\IFF\quad
