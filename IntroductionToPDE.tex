...
\newpage
\section{Spherical averages}\label{sec67bac9ff}
...
\begin{proof}
	...
	\begin{align*}
		...
		&= \frac{1}{\omega_n  (\theta r)^n} \left| \int_{\de B(x,\theta r)} u(y) e_j\cdot \frac{y}{|y|} \did S(y) \right| \\
...
\begin{proof}
	...
	\begin{align*}
		...
		&= \int_{\de V_\epsilon} \grad u(y) G(x,y)\cdot\nu_{V_\epsilon}(y) \did y
			- \int_{\de V_\epsilon} u(y)  \grad_y G(x,y)\cdot\nu_{V_\epsilon}(y) \did y \\
		&= - \int_{\de B(x,\epsilon)} \grad u(y) G(x,y) \cdot\frac{y-x}{|y-x|} \did y
			+ \int_{\de U} \grad u(y) \underbrace{G(x,y)}_{=0} \cdot\nu_U(y) \did y \\
		&\qquad
			- \int_{\de U} u(y) \grad_y G(x,y) \did y 
			+ \int_{\de B(x,\epsilon)} u(y) \grad_y G(x,y)\cdot\frac{y-x}{|y-x|} \did y \\
		&= - \int_{\de B(x,\epsilon)} \grad u(y) G(x,y) \cdot\frac{y-x}{|y-x|} \did y
			- \int_{\de U} u(y) \grad_y G(x,y) \did y \\ 
		&\qquad
...
\begin{theorem}[Dirichlet's principle]\label{thm67c2da70}
	...
	\item\label{thm67c2da70_ii}
		...
		$u$ is the arg-min of $\dirichletE_f$ on $\scr A_g$.
	\end{enumerate}
\end{theorem}
\begin{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Symmetries of the heat operator}
...
Then $\tilde bar u\in C^2(O^{-1}(U-b)\times (I-\tau))$ and
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Fundamental solution for the heat equation}
...
\begin{theorem}\label{thm67c30be5}
	...
	\begin{equation}
		...
		= \frac1{(4\pi t)^{n/2}} \int_{\R^n} \exp\left( -\frac{|x|^2}{4t} \right) g(y) \did y ,
	\end{equation}
	for $x\in\R^n$ and $t>0$.
	\begin{enumerate}
	\item\label{thm67c30be5_smooth}
		$u\in C^\infty(\R^n\times (0,+\infty))$ and, for every $\alpha\in\N^n$,
		\begin{equation}\label{eq67c33d51}
			\Diff^\alpha u(x,t) = \int_{\R^n} \Diff_x^\alpha\Phi(x-y,t) g(y) \did y .
		\end{equation}
...
\begin{proof}
	...
	\[
	K(x,t;y) = \frac1{(4\pi t)^{n/2}} \exp\left( -\frac{|x|^2}{4t} \right) g(y) .
	\]
...
\begin{lemma}\label{lem67c41d25}
	...
	\begin{equation}\label{eq67c41b8a}
		...
		\int_{\R^n} \Phi(z,t) g(\hat x - z,t) \did z = g(\hat x,0) .
	\end{equation}
\end{lemma}
\begin{exercise}
...
\begin{proof}
	...
	\end{equation}
	Fix $\hat x\in\R^n$ and $\epsilon>0$.
	Since $g$ is continuous, there exists $\delta>0$ such that $|g(y,t)-g(\hat x,t)|<\epsilon$ for all $y\in B(\hat x,\delta)$ and all $t\in[0,T]$.
	Then
	\begin{align*}
	 	|u(x,t)-g(\hat x,t)|
		&= \left| \int_{\R^n} \Phi(x-y,t) g(y,t) \did y - g(\hat x,t) \int_{\R^n} \Phi(x-y,t)\did y \right| \\
		&\le \int_{\R^n} \Phi(x-y,t) |g(y,t)-g(\hat x,t)| \did y \\
		&= \int_{B(\hat x,\delta)} \Phi(x-y,t) \underbrace{|g(y,t)-g(\hat x,t)|}_{<\epsilon} \did y \\
		&\quad + \int_{\R^n\setminus B(\hat x,\delta)} \Phi(x-y,t) |g(y,t)-g(\hat x,t)| \did y \\
		&\le \epsilon \underbrace{\int_{\R^n} \Phi(x-y,t) \did y}_{=1}
			+ \underbrace{ \int_{\R^n\setminus B(\hat x,\delta)} \Phi(x-y,t) |g(y,t)-g(\hat x,t)| \did y }_{=: J_\delta(x)} .
	\end{align*}
	If $x\in B(\hat x,\delta/2)$, then, for all $y\in \R^n\setminus B(\hat x,\delta)$,
...
The construction of a solution for the nonhomogeneous Cauchy problem~\eqref{eq67c344a9} is constructed as follows.
...
\begin{proof}
	...
	\begin{align*}
		...
		&\qquad
			- \int_\epsilon^t \int_{\R^n} (\div_z(\Phi(z,r)\grad_zf(x-z,t-r) - \grad_z\Phi(z,r)f(x-z,t-r)) \did z\did r \\
		&\qquad
...
\begin{theorem}[Strong maximum principle]\label{thm67c44b0d}
	...
	\end{equation}
	Moreover, if $U$ is connected and there exists $(x,t)\in U_T$ such that $u(x,t) = \max_{\bar U_T} u$, then $u$ is constant on $\bar U_t$.
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
\TODO: Recheck the proof!

	Set $g\in C^0(\R^n)$ by $g(x) = u(x,0)$.
	
	Fix $\hat y\in\R^n$ and $\mu>0$.
	...
	\begin{equation}
		...
		0<b<\frac1{4a} .
	\end{equation}
	Define, for $x\in\R^n$ and $t\in(0,b)$,
	\begin{equation}\label{eq67c4590e}
		v(x,t) = u(x,t) - \frac{\mu}{(b-t)^{n/2}} \exp\left( \frac{|x-y|^2}{4(b-t)} \right) .
	\end{equation}
	So, we have 
	...
	By the strong maximum principle~\ref{},
	for every $r>0$,
	\begin{equation}
		\sup\left\{ v(x,t) : 
		x\in B(y,r),\ t\in[0,b)
		...
		=
		\sup\left\{ v(x,t) : 
		x\in B(y,r)\text{ and }t=0,
		\text{ or }
		x\in\de B(y,r)\text{ and }t\in[0,b)
		\right\} .
	\end{equation}
	If $|x-y|\le r$, then 
	$v(x,0) = u(x,0) - \frac{\mu}{b^{n/2}} \exp\left(  \frac{|x-y|^2}{4b} \right)
	\le g(x) \le \|g\|_{L^\infty}$.
	
	If $|x-y|=r$ and $t\in[0,b)$, then 
	\begin{align*}
	 	v(x,t) 
		&= u(x,t) - \frac{\mu}{(b-t)^{n/2}} \exp\left( \frac{r^2}{4(b-t)} \right) \\
		&\le A\exp(a|x|^2) - \frac{\mu}{b^{n/2}} \exp\left(  \frac{r^2}{4b} \right) \\
		&\le A\exp(a(|y|+r)^2) - \frac{\mu}{b^{n/2}} \exp\left(  \frac{r^2}{4b} \right) \\
		&= \exp(a(|y|+r)^2) \left( A - \frac{\mu}{b^{n/2}} \exp\left(  \frac{r^2}{4b} - a (|y|+r)^2 \right) \right) =: J_r .
	\end{align*}
	Since $\lim_{r\to\infty} J_r = -\infty$, 
	we can choose $r$ large enough that the $J_r < \|g\|_{L^\infty}$.
	We conclude that, for $t\in[0,b)$,
	\begin{equation}
		u(\hat y,t) - \frac{\mu}{(b-t)^{n/2}}
		= v(\hat y,t)
		\le \sup\{ u(x,t) : x\in B(\hat y,r), t\in[0,b)\} 
		\le J_r \le \|g\|_{L^\infty} .
	\end{equation}
	So, if we fix $t\in [0,b)$, then 
	$u(\hat y,t) = \lim_{\mu\to0} u(\hat y,t) - \frac{\mu}{(b-t)^{n/2}} \le \|g\|_{L^\infty}$.
	We can then say that $u(y,t)\le u(y,t-b)$ for all $t\in [b,T]$.
...
\begin{theorem}[Uniqueness for the unbounded Cauchy problem]\label{thm67c45cb9}
	...
	\begin{equation}\label{eq67c45e49}
		...
		: \exists A,a>0,\ |u(x,t)| \le A\exp(a|x|^2)
		\right\} .
	\end{equation}
...
A \emph{topological vector space} is a pair $(V,\tau)$ where $V$ is a vector space over $\K\in\{\R,\C\}$ and $\tau$ is a topology on $V$ such that
\begin{enumerate}
\item	The sum operation $+:V\times V\to V$ is continuous;
\item	The inverse operation $-:V\to V$ is continuous;
\item	The scalar multiplication $\cdot:\K\times V\to V$ is continuous.
\end{enumerate}
We will usually just write $V$ for $(V,\tau)$.




\subsection{Dual space}
If $V$ is a vector space over $\K\in\{\R,\C\}$, then 
we have an \emph{algebraic dual space}
\[
V^* = \{f:V\to \K\text{ linear}\}
\]
and a \emph{topological dual} (or just \emph{dual}) 
\[
V' = \{f\in V^*\text{ continuous}\} .
\]
If $V$ is finite dimensional, then $V'=V^*$.

\subsection{Continuous bilinear maps}
Let $V$ be a vector space over $\K\in\{\R,\C\}$ and $B\in \Lin^2(V;\K)$ be a continuous bilinear map.
Define a function $A_B:V\to V'$ by
\[
\langle A_B(v) | w \rangle = B(v,w) .
\]
Then $A_B$ is a linear continuous operator.

Linearity is trivial.
Continuity is proven using nets: 
If $\{v_n\}_{n\in\scr N}\subset V$ is a net converging to $v_\infty\in V$,
then, for every $w\in W$, $A_B(v_n)w \to A_B(v_\infty)w$, by the continuity of $B$.

In fact, we also have a uniform continuity: if $v_n\to v_\infty$ and $w_n\to w_\infty$, then $A_B(v_n)w_n\to A_B(v_\infty)w_\infty$.

If $V$ is a Banach space with norm $\|\cdot\|$, and $B$ is strongly continuous, that is, there exists $C>0$ such that
\begin{equation}\label{eq67aa7654}
|B[v,w]| \le C \|v\| \|w\| ,
\end{equation}
 then $A_B$ is strongly continuous.
Indeed, 
\[
\|A_B(v)\| 
= \sup_{\|w\|\le 1} \|B[v,w]\|
...
\begin{equation}\label{eq67aa6d2b}
|B[v,v]| \ge c \|v\|^2 ,
\end{equation}
then $A_B$ is injective.
%First,~\eqref{eq67aa6d2b} implies that $A_B$ is injective.
Indeed, if $A_Bv=0$, then $0 = A_B(v)v \overset{\eqref{eq67aa6d2b}}\ge c \|v\|^2$ and thus $v=0$.

If in addition to~\eqref{eq67aa7654} and~\eqref{eq67aa6d2b},
we assume that $V$ is locally convex and reflexive, then $A_B$ is an isomorphism.
(See Lemma~\ref{lem67aa726c}.)

%Second,~\eqref{eq67aa6d2b} implies that $A_B$ is surjective.
For this aim, define $\||v\|| := \sqrt{|B[v,v]|}$.
Then, $\||\cdot\||$ is a norm, because
\begin{align*}
	\||av+bw\|| 
	&\le \sqrt{|a|^2 |B[v,v]| + |ab| (|B[v,w]|+|B[w,v]|) + |b|^2 |B[w,w]|} \\
	&\le \sqrt{|a|^2 |B[v,v]| + 2 |ab| C \|v\|\|w\| + |b|^2 |B[w,w]|} \\
	&\le \sqrt{|a|^2 |B[v,v]| + 2 |ab| C/c \sqrt{|B[v,v]||B[w,w]|} + |b|^2 |B[w,w]|} \\
	&\le \max\{\sqrt{C/c},1\} \sqrt{|a|^2 |B[v,v]| + 2 |ab| \sqrt{|B[v,v]||B[w,w]|} + |b|^2 |B[w,w]|} \\
	&= \max\{\sqrt{C/c},1\} \sqrt{ (|a|\sqrt{|B[v,v]|} + |b| \sqrt{|B[w,w]|} )^2 } \\
	&= \max\{\sqrt{C/c},1\} (|a| \||v\|| + |b| \||w\||) .
\end{align*}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Reflexivity}

\begin{lemma}\label{lem67aa73f6}
	Let $V$ be a topological vector space and $\alpha,\beta\in V'$ such that $\ker(\alpha)=\ker(\beta)$.
	Then $\alpha$ and $\beta$ are linearly dependent.
\end{lemma}
\begin{proof}
	Let $W:=\ker(\alpha)$ and suppose $W\neq V$.
	Let $\hat v\notin W$.
	We claim that $V=W\oplus\K\hat v$.
	Indeed, if $ v\in V\setminus W$, then
	there is $(a,b)$ with $ab\neq0$ such that
	$\alpha(a\hat v + b  v) = a \alpha(\hat v) + b \alpha( v) = 0$
	and thus $a\hat v + b v\in W$, i.e., $v \in  W + \frac{a}{b} \hat v$.
	
	So, returning to the original statement, $\alpha$ and $\beta$ are determined by their values on $\hat v$, and thus they are linearly dependent.
\end{proof}

\begin{lemma}\label{lem67aa726c}
	Let $V$ be a locally convex topological vector space.
	The following statements are equivalent
	\begin{enumerate}[label=(\roman*)]
	\item\label{lem67aa726c_1}
	$V$ is reflexive, that is, the canonical embedding $\iota:V\to V''$, $\langle \iota(v)|\alpha \rangle = \langle \alpha|v \rangle$ for all $\alpha\in V'$, is an isomorphism.
	\item\label{lem67aa726c_2}
	For every closed subspace  $W\lhd V'$ of the dual, there exists $v\in V\setminus\{0\}$ such that $\langle \alpha|w \rangle=0$ for all $\alpha\in W$.
	\end{enumerate}
\end{lemma}
\begin{proof}
	$\ref{lem67aa726c_1}\THEN\ref{lem67aa726c_2}$.
	Let $W\lhd V'$ be closed.
	Define $\alpha:W\to \K$ by setting $\alpha(W)=\{0\}$.
	Let $\hat w\in V'\setminus W$ and set $\alpha(\hat w) = 1$.
	Then, by the Hahn--Banach Theorem~\ref{} $\alpha$ can be extended to a linear continuous function on the whole $V'$, that is, $\alpha\in V''$.
	Since $V$ is assumed to be reflexive, $\alpha=\iota(v)$ for some $v$, and such $v$ is the one aimed at.
	
	$\ref{lem67aa726c_2}\THEN\ref{lem67aa726c_1}$.
	Suppose $V$ is not reflexive.
	Then there exists $\alpha\in V''\setminus \iota(V)$.
	Let $W = \ker(\alpha)$.
	We clearly have that $W\lhd V'$ is a closed subspace.
	If $v\in V$ is such that $W\subset\ker(\iota(v))$, then, by Lemma~\ref{lem67aa73f6} there is $\lambda\in\K$ such that $\iota(v) = \lambda\alpha$.
	Since $\alpha\notin\iota(V)$, then $\lambda=0$ and $v=0$.
\end{proof}
